...
export default function CommentsBoard({ messages, userId, defaultAvatar }: CommentsBoardProps) {
                ...
                <div className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'}`} key={msg.id}>
                  <div className={`flex items-start space-x-3 max-w-xs ${isOwnMessage ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    ...
                    <div className={`rounded-2xl px-4 py-3 shadow-md ${
                      ...
                      <div className={`text-xs mb-1 ${isOwnMessage ? 'opacity-80' : 'text-gray-500'}`}>
                      ...
                      {shouldShowExpandButton && (
                        <button
                          ...
                          className={`mt-2 flex items-center gap-1 text-xs font-medium transition-colors duration-200 ${
                            isOwnMessage ? 'text-white/80 hover:text-white' : 'text-purple-600 hover:text-purple-700'
                          }`}
                        >
                          {isExpanded ? "收起" : "展开"}
                          <svg
